...
func TestVerifyConfig(t *testing.T) {
	...
	runs := []struct {
	...
	}{
		{
			...
			out:        "An error occured while extracting the file: open config_test/apid.yml: no such file or directory",
			BeforeWork: func() {},
		},
		{
			name: "prints file is empty",
			args: argsWithDirectory,
			out:  "An error occured while extracting the file: The file apid.yml is completely empty. What do you want me to do with this?",
			BeforeWork: func() {
		...
		},
		{
			...
			args: argsWithDirectory,
			out:  "An error occured while extracting the file: yaml: unmarshal errors:\n  line 1: cannot unmarshal !!str `b` into config.Config",
			BeforeWork: func() {
...
database:
  ...
  password: '********'`, out, "print to screen did not match expected config")
	...
	t.Run("prints error(s) to screen", func(t *testing.T) {
		...
		assert.Equal(t, `Error(s) were found while parsing config_test/apid.yml, please address them:
