...
Global Flags:
  ...
  -s, --secrets            Print secrets`
	...
	errorsWereFoundVerifying := "Error(s) were found while verifying the config, please address them:\n"
	...
	runs := []struct {
		...
		},
		{
			...
			out:  "Could not write file to dir_not_in_existance/apid.yml",
		},
		{
			name: "will fail to write file because there is no directory with debug",
			args: []string{"config", "create", "--debug", "--directory=dir_not_in_existance", "--database_password=test_password", "--database_username=test_username"},
			out:  "Error: open dir_not_in_existance/apid.yml: no such file or directory\nCould not write file to dir_not_in_existance/apid.yml",
		},
		{
			name: "an invalid filename will cause an issue",
			args: append(argsWithDatabaseSet, "--filename=apid"),
			out:  errorsWereFoundVerifying + "  - The filename is not in the form [NAME].yml",
		},
	}
	...
	for _, run := range runs {
		t.Run(run.name, func(t *testing.T) {
			...
			assert.Equal(t, out, run.out, "unexpected manager output")
	...
	t.Run("prints multiple issues", func(t *testing.T) {
		...
		assert.NoError(t, err, "expected no error running manager")
		assert.Equal(t, `Error(s) were found while verifying the config, please address them:
