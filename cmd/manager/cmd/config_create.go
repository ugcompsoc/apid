...
func CreateConfigPreRun(cmd *cobra.Command, args []string) {
	isInteractive, _ := createConfigCmd.Flags().GetBool("interactive")
	if !isInteractive {
...
func CreateConfig(cmd *cobra.Command, args []string) {
	...
	issues, err = c.Verify()
	// Unable to reach this currently as an error is only generated when
	// the regex is invalid.
	if err != nil {
		if debug {
			cmd.Printf("Error: %s\n", err)
		}
		cmd.Printf("An error was encountered while verifing the config\n")
		return
	}
	...
	if !filenameRegex.MatchString(filename) {
		issues = append(issues, "The filename is not in the form [NAME].yml")
	}

	if len(issues) != 0 {
		cmd.Print("Error(s) were found while verifying the config, please address them:\n")
		for _, issue := range issues {
			cmd.Printf("  - %s\n", issue)
		}
		return
	...
	if err != nil {
		...
		cmd.Printf("Could not write file to %s\n", absoluteFilePath)
		return
	}
	...
	if print {
		...
		cYaml, err = yaml.Marshal(c)
		// Same craic as the marshal above
		if err != nil {
			if debug {
				cmd.Printf("Error: %s\n", err)
			}
